...
# Inspired by https://github.com/sthalles/SimCLR-tensorflow/blob/master/models/cnn_small.py


class SmallCNN(tf.keras.Model):
    def __init__(self, out_dim):
        super(SmallCNN, self).__init__()
        self.conv1 = tf.keras.layers.Conv2D(filters=16, kernel_size=3, padding='same', strides=1)
        self.conv2 = tf.keras.layers.Conv2D(filters=32, kernel_size=3, padding='same', strides=1)
        self.conv3 = tf.keras.layers.Conv2D(filters=64, kernel_size=3, padding='same', strides=1)
